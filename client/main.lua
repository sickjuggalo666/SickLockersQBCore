...
AddEventHandler('SickEvidence:triggerEvidenceMenu', function()
	...
	QBCore.Functions.TriggerCallback('SickEvidence:getInventory', function(exists)
		if not exists then
			lib.registerContext({
				id = 'confirmCreate',
...
AddEventHandler('SickEvidence:lockerCallbackEvent', function()
    QBCore.Functions.TriggerCallback('SickEvidence:getPlayerName', function(data)
        if data ~= nil then
			local lockerID = ("LEO: "..data.firstname.." "..data.lastname)
			QBCore.Functions.TriggerCallback('SickEvidence:getLocker', function(locker)
				if locker then
					local lockerID = ("LEO: "..data.firstname.." "..data.lastname)
					lib.registerContext({
						id = 'lockerCreate',
...
AddEventHandler('SickEvidence:ChiefLookup', function()
	...
	QBCore.Functions.TriggerCallback('SickEvidence:getLocker', function(exists)
		if not exists then
			lib.registerContext({
				id = 'lockerOption',
				title = 'Confirm or Cancel',
...
AddEventHandler('SickEvidence:OtherlockerCallbackEvent', function()
    QBCore.Functions.TriggerCallback('SickEvidence:getPlayerName', function(data)
        if data ~= nil then
			...
			QBCore.Functions.TriggerCallback('SickEvidence:getOtherInventories', function(Otherlocker)
				if Otherlocker then
					local OtherlockerID = (PlayerData.job.label.. ": " ..data.firstname.." "..data.lastname)
					lib.registerContext({
...
local function refreshjob()
    Citizen.Wait(1)
    PlayerData = QBCore.local functions.GetPlayerData()
end